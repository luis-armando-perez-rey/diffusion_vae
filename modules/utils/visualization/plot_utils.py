...
def labels_to_angles(labels):
    """
    Transforms integer labels to angles
    :param labels: integer labels
    :return: array of corresponding angles
    """
    normalized = labels/np.amax(labels)
...
def YIQ_embedding_continuous(phi):
    result = np.zeros(phi.shape + (3,))
    result[..., 0] = 0.4 - 0.2 * np.sin(phi)
    result[..., 1] = 0.25 * np.cos(phi)
    result[..., 2] = 0.25 * np.sin(phi)
    print(result.shape)
    return yiq_to_rgb(result.reshape((-1, 3)))
